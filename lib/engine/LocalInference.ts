...
const runLocalCompletion = async (
    ...
    // ── Token batching ────────────────────────────────────────────────────────
    // En lugar de actualizar el estado React en cada token (re-render costoso),
    // acumulamos tokens en una variable JS pura y flusheamos cada ~66ms (15fps).
    // Esto reduce drásticamente el bloqueo del JS thread durante la inferencia.
    const UI_FLUSH_MS = 66
    let _tokenBatch = ''
    let _lastFlush = 0
    let _ttsBuffer = ''
    ...
    const flushBatch = () => {
        if (_tokenBatch) {
            Chats.useChatState.getState().insertToBuffer(_tokenBatch)
            _tokenBatch = ''
        }
        _lastFlush = Date.now()
    }

    // Silenciar logs durante inferencia para no bloquear JS thread
    Logger.setInferencing(true)

    let reasoningMode = false
    const outputStream = (text: string) => {
        const cleaned = cleanStopString(text)

        // Acumular en batch puro (sin React state)
        _tokenBatch += cleaned

        // Flush al estado React solo cada 66ms
        const now = Date.now()
        if (now - _lastFlush >= UI_FLUSH_MS) {
            flushBatch()
        }

        // Tracking de reasoning mode para think tags
        if (reasoningMode) {
            if (isCloseThinkTag(cleaned)) reasoningMode = false
            return
        }
    ...
    const outputCompleted = (text: string, timings: CompletionTimings) => {
        // Flush cualquier batch pendiente antes de terminar
        flushBatch()
        Logger.setInferencing(false)
